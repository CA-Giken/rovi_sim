...
def do_solve():
  global capt_count,sol_count
  if sol_count<capt_count:
    sol_count=capt_count
    rospy.set_param('/vloader/index',Param["index"]+1)
    do_move()
...
def cb_solve():
  rospy.Timer(rospy.Duration(3),lambda ev: do_solve(),oneshot=True)
...
rospy.Subscriber("/request/clear",Bool,lambda f:cb_clear())
rospy.Subscriber("/logger/prepare",Bool,lambda f:cb_clear())
rospy.Subscriber("/request/solve",Bool,lambda f:cb_solve())
