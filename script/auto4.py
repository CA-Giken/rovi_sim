...
def captMov():
  ...
  bTc[0,3]=pos[0]
  bTc[1,3]=pos[1]-(0 if 'prob_x' not in stats else stats['prob_x'])
...
def cb_capture(msg):
  global mode,retry,merge,yindex,stats
  print("cb_capture",merge)
  merge=merge+1
  if "probables" in stats:
...
def cb_solve(msg):
  ...
  print("cb_solve",mode,merge)
  if msg.data:
    if stats["prob_m"]<150:     #Margin to the post
      print("auto pick2")
